...
S : E
;
E : START {if(pass_no==2){fprintf(yyout,"%s",yytext);}} STAT1 END{if(pass_no==2){fprintf(yyout,"%s",yytext);}}
;
STAT1 : ST STAT1
|
;
ST : REGOPR {if(pass_no==2){fprintf(yyout,"%s",yytext);}} 
| OPRTYPE1 {if(pass_no==2){fprintf(yyout,"%s",yytext);}}
| TYPE1 {if(pass_no==2){fprintf(yyout,"%s",yytext);}}
| OPRTYPE2 {if(pass_no==2){fprintf(yyout,"%s",yytext);}}
| BYTE {if(pass_no==2){fprintf(yyout,"%s",yytext);}}
| WORD {if(pass_no==1){addvariable(yytext);}
        else if(pass_no==2){fprintf(yyout,"%s",yytext);}}
| RESERVE {if(pass_no==2){fprintf(yyout,"%s",yytext);}}
| EQU {if(pass_no==2){fprintf(yyout,"%s",yytext);}}
| ORG {if(pass_no==2){fprintf(yyout,"%s",yytext);}}
| SINGMAC
| MAC 
| {if(pass_no==1)addarg(yytext); 
	}   MACROCALL {if(pass_no==2)replacecall(yytext);}
;
SINGMAC :{ if(pass_no==1)
		...
		SMAC {if(pass_no==1){strcpy(DEFTAB[line_count],head);line_count++;head_count=0;}}STAT2 
		  {if(pass_no==1){strcpy(DEFTAB[line_count],yytext);line_count++;}}
		   MEND{if(pass_no==1){NAMTAB[count].defEnd=line_count-1;count++;}
				}
MAC : { if(pass_no==1)
		{check(yytext);
		defnparametercount=0;
		...
		LABEL WS MACRO WHITE PARAMS {if(pass_no==1){strcpy(DEFTAB[line_count],head);line_count++;head_count=0;}}
	    STAT2 {if(pass_no==1){strcpy(DEFTAB[line_count],yytext);line_count++;}}
	    MEND {if(pass_no==1){NAMTAB[count].defEnd=line_count-1;count++;}
				}
;
STAT2 : {if(pass_no==1){strcpy(DEFTAB[line_count],yytext); if(strstr(yytext,":"))addlbl(yytext);line_count++; } }ST2 STAT2 
